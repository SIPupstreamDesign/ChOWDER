 ...
 * 旧実装 iframe_connector.js の TypeScript ポート
 */
export class IFrameConnector {
    ...
    connect(callback: () => void): void {
        ...
        // 親モード: Init を iframe に送り、即座に callback を呼ぶ
        this.send('Init', {}, () => {});
        callback();
    }
    ...
    sendResponse(request: any, args: any = {}): void {
        ...
        try {
            if (this.contentWindow_) {
                this.contentWindow_.postMessage(JSON.stringify(resjson), location.href);
            }
        } catch (e) {
            console.error('[IFrameConnector] sendResponse error:', e);
        }
    }
