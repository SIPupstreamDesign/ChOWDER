...
class MapDialog extends EventEmitter {
    ...
    init() {
        ...
        this.dom.className = "map_dialog";

        this.wrap = document.createElement('div');
        this.wrap.className = "map_dialog_wrap";

        let createRow = () => {
            let row = document.createElement('div');
            row.className = "map_dialog_row";
            this.wrap.appendChild(row);
            return row;
        }

        this.title = document.createElement('p');
        this.title.className = "map_dialog_title";
        this.title.innerText = "地図追加";

        this.idTitle = document.createElement('p');
        this.idTitle.className = "map_dialog_id_title";
        this.idTitle.innerText = "ID:";

        this.idInput = new Input("text");
        this.idInput.getDOM().className = "map_dialog_id_input";
        this.idInput.setValue("Map_" + Math.floor(Math.random() * 100));


        this.urlTitle = document.createElement('p');
        this.urlTitle.className = "map_dialog_url_title";
        this.urlTitle.innerText = "URL:";

        this.urlInput = new Input("text");
        this.urlInput.getDOM().className = "map_dialog_url_input";
        this.urlInput.setValue("http://localhost:8080/std/${z}/${x}/${y}.png");

        this.zoomMinTitle = document.createElement('p');
        this.zoomMinTitle.className = "map_dialog_zoom_title";
        this.zoomMinTitle.innerText = "ZOOM: Min";

        this.zoomMaxTitle = document.createElement('p');
        this.zoomMaxTitle.className = "map_dialog_zoom_title map_dialog_zoom_max_title";
        ...
        this.zoomMinSelect.getDOM().className = "map_dialog_zoom_min_select";

        this.zoomMaxSelect = new Select();
        for (let i = 1; i <= 20; ++i) {
            this.zoomMaxSelect.addOption(i, String(i));
        }
        this.zoomMaxSelect.getDOM().className = "map_dialog_zoom_max_select";
        this.zoomMaxSelect.setSelectedIndex(this.zoomMaxSelect.getOptions().length - 1);

        this.okButton = new Button();
        this.okButton.setDataKey("OK");
        this.okButton.getDOM().className = "map_dialog_ok_button btn btn-primary";
        this.dom.appendChild(this.okButton.getDOM());

        this.cancelButton = new Button();
        this.cancelButton.setDataKey("Cancel");
        this.cancelButton.getDOM().className = "map_dialog_cancel_button btn btn-light";
