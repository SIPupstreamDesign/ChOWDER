...
class Display {
    ...
    updateIFrameRect(metaData) {
        // webgl iframeの更新
        if (metaData.type === Constants.TypeWebGL || metaData.type == Constants.TypeTileViewer) {
            ...
            if (elem) {
                if (elem.children[0] && elem.children[0].nodeName.toLowerCase() === "iframe") {
                    if (funcDict.hasOwnProperty(metaData.id)) {
                        if (funcDict[metaData.id].hasOwnProperty('chowder_itowns_resize')) {
                            funcDict[metaData.id].chowder_itowns_resize(rect);
                        }
                    }
                }
                if (funcDict.hasOwnProperty(metaData.id)) {
                    if (funcDict[metaData.id].hasOwnProperty('chowder_tileviewer_resize')) {
                        funcDict[metaData.id].chowder_tileviewer_resize(metaData, rect);
                    }
                }
            }
        }
                ...
                if (isUpdateContent || (!isWindow && Validator.isVisible(json))) {
                    if (isUpdateContent && json.type !== "tileimage") {
                        ...
                        if (!isUpdateContent && elem) {
                            this.gui.assignTileImage(json, false, ((json) => {})(json));
                        } else {
                            // 新規コンテンツロード.
                            // 全タイル読み込み済じゃなかったら返る
    ...
    initEvent() {
        ...
        this.store.on(Store.EVENT_DONE_ADD_TILEVIEWER_FUNC, (err, id) => {
            let metaData = this.store.getMetaData(id);
            // 読み込み完了までテンポラリで枠を表示してる．枠であった場合は消す.
