...
class InputDialog {
     ...
     */
    static showTextInput(setting, okCallback) {
        let input_dialog,
            input_dialog_name,
            input_dialog_input,
            okbutton,
     ...
     * @param setting.backgroundColor 背景色
     */
    static showMultiTextInput(setting, callback) {
        let text_input_dialog,
            dialog_label,
            textarea,
            ok_button,
            background = new PopupBackground(),
        ...
        */
        ...
        ok_button.onclick = function(evt) {
            if (callback) {
                let width = (textarea.clientWidth + 1);
                let height = (textarea.clientHeight + 1);
     ...
     * @param setting.backgroundColor 背景色
     */
    static showColorInput(setting, okCallback) {
        let color_dialog,
            dialogname,
            color_picker,
            okbutton,
        ...
        */
        ...
        let colorselector = new ColorSelector(function(colorvalue) {}, 234, 120); // 幅、高さ
        color_picker.appendChild(colorselector.elementWrapper);
        if (setting.initialValue) {
            let col = setting.initialValue.split('rgb(').join("");
            col = col.split(")").join("");
            col = col.split(",");
     ...
     * @param setting.backgroundColor 背景色
     */
    static showOKCancelInput(setting, callback) {
        let dialog,
            ok_button,
            cancel_button,
            dialog_div,
     ...
     * @param setting.backgroundColor 背景色
     */
    static showYesNoCancelInput(setting, callback) {
        let dialog,
            yes_button,
            no_button,
            cancel_button,
     ...
     * @param setting.backgroundColor 背景色
     */
    static showOKInput(setting, callback) {
        let dialog,
            ok_button,
            dialog_div,
            background = new PopupBackground(),
