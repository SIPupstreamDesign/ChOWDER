 ...
 */
    ...
    _setAdjacencyList() {
        ...
        //切り捨てるスキャン回数を決定
        /*for (let adjacencyId in this.AdjacencyList) {
            if (!scanMinMargin[adjacencyId]) {
                scanMinMargin[adjacencyId] = [];
            }
            scanMinMargin[adjacencyId]["up"] = 0;
            scanMinMargin[adjacencyId]["down"] = 0;
            scanMinMargin[adjacencyId]["right"] = 0;
            scanMinMargin[adjacencyId]["left"] = 0;
            for (let upId in this.AdjacencyList[adjacencyId].adjacencyUp) {
                scanMinMargin[adjacencyId]["up"] += this.AdjacencyList[adjacencyId].adjacencyUp[upId];
            }
            for (let downId in this.AdjacencyList[adjacencyId].adjacencyDown) {
                scanMinMargin[adjacencyId]["down"] += this.AdjacencyList[adjacencyId].adjacencyDown[downId];
            }
            for (let rightId in this.AdjacencyList[adjacencyId].adjacencyRight) {
                scanMinMargin[adjacencyId]["right"] += this.AdjacencyList[adjacencyId].adjacencyRight[rightId];
            }
            for (let leftId in this.AdjacencyList[adjacencyId].adjacencyLeft) {
                scanMinMargin[adjacencyId]["left"] += this.AdjacencyList[adjacencyId].adjacencyLeft[leftId];
            }
        };
        console.log(scanMinMargin);
        for (let id in scanMinMargin) {
            for (let markerId in scanMinMargin[id]) {
                scanMinMargin[id][markerId] = scanMinMargin[id][markerId] / 2;
            }
        }
*/
        //各方向のスキャンした回数から隣接関係を決定
        /* for (let i in this.AdjacencyList) {
             for (let j in this.AdjacencyList) {
                 if (this.AdjacencyList[i]) {
                     ...
                     if (this.AdjacencyList[i].adjacencyUp[j]) {
                         ...
                         let upIdPoint = this.deepCopy(this.AdjacencyList[i].adjacencyUp[j]);
                         console.log("tmp2")
                         console.log(scanMinMargin[i]["up"]);
                         if (upIdPoint > this.AdjacencyList[i].adjacency["up"] && upIdPoint > scanMinMargin[i]["up"]) {
                             console.log("UP")
                             this.AdjacencyList[i].adjacency["up"] = j;
                         }
                     }
                     ...
                     if (this.AdjacencyList[i].adjacencyDown[j]) {
                         let downIdPoint = this.deepCopy(this.AdjacencyList[i].adjacencyDown[j]);
                         if (downIdPoint > this.AdjacencyList[i].adjacency["down"] && downIdPoint > scanMinMargin[i]["down"]) {
                             console.log("DOWN")
                             this.AdjacencyList[i].adjacency["down"] = j;
                         }
                     }
                     ...
                     if (this.AdjacencyList[i].adjacencyRight[j]) {
                         let rightIdPoint = this.deepCopy(this.AdjacencyList[i].adjacencyRight[j]);
                         if (rightIdPoint > this.AdjacencyList[i].adjacency["right"] && rightIdPoint > scanMinMargin[i]["right"]) {
                             console.log("RIGHT")
                             this.AdjacencyList[i].adjacency["right"] = j;
                         }
                     }
                     ...
                     if (this.AdjacencyList[i].adjacencyLeft[j]) {
                         let leftIdPoint = this.deepCopy(this.AdjacencyList[i].adjacencyLeft[j]);
                         if (leftIdPoint > this.AdjacencyList[i].adjacency["left"] && leftIdPoint > scanMinMargin[i]["left"]) {
                             console.log("LEFT")
                             this.AdjacencyList[i].adjacency["left"] = j;
                         }
                     }
        ...
        for (let i in this.AdjacencyList) {
            ...
            for (let j = 0; j < 2; j++) {
                if (this.AdjacencyList[i].adjacency[directionName[2*j]] !== -1 && this.AdjacencyList[i].adjacency[directionName[2*j + 1]] === -1) {
                    ...
                    while (id !== -1) {
                        resultCount[directionName[2*j]][id] += count;
                        console.log(resultCount[directionName[2*j]][id]);
                        id = this.AdjacencyList[id].adjacency[directionName[2*j]];
                        count++
                    }
                }
